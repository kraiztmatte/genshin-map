...
function onEachFeature(feature, layer) {
  ...
  popupHtml +=
    '<div class="myPopName" >' +
    feature.properties.popTitle +
    feature.id +
    "</div>";
  var switchClass = !localStorage.getItem(key)
    ? "myPopSwitchTodo"
    : "myPopSwitchDone";
  popupHtml +=
    '<div class="' +
    switchClass +
    '" onclick="MarkPoint(this)" data-key="' +
    key +
    '"></div>';
  popupHtml += "</div>";
  ...
  popupHtml +=
    "<img src=./comment_png/" +
    key +
    ".png onerror=\"this.src='./imgs/Icon_51.png'\">";
  popupHtml += "</div>";
  popupHtml += "</div>";
...
function MarkPoint(element) {
  ...
  if (newValue) {
    $(".leaflet-marker-icon.mark-" + key).attr(
      "src",
      "./imgs/icon_" + layerNumber + "_done.png"
    );
...
map.on("popupopen", function (e) {
  ...
  popupHtml +=
    '<div class="myPopName" >' +
    marker.feature.properties.popTitle +
    marker.feature.id +
    "</div>";
  popupHtml +=
    '<div class="' +
